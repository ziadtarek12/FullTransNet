...
# class AverageMeter(object):
#     def __init__(self, *keys: str):
#         self.max_values = {key: float('-inf') for key in keys}
#
#     def update(self, **kwargs: float) -> None:
#         for key, value in kwargs.items():
#             self._check_attr(key)
...
#     def __getattr__(self, attr: str) -> float:
#         self._check_attr(attr)
#         return self.max_values[attr]  # 直接返回最大值
#
#     def _check_attr(self, attr: str) -> None:
#         assert attr in self.max_values, f"The key '{attr}' is not defined. Please initialize it first."
#
...
if __name__ == '__main__':
    ovp_user_summary_path = '/home/jianglu/test/Ushape_videosum/splits/ovp_usersummary_org/'
    video_name = 'video_50'
    n_frames = 1400
