...
def get_summ_f1score_tensor(pred_summ: torch.Tensor,
                     test_summ: torch.Tensor,
                     eval_metric: str = 'avg'
                     ) -> float:
    ...
    if pred_summ.size(0) > n_frames:
        ...
        pred_summ = F.pad(pred_summ, (0, n_frames - pred_summ.size(0)))

    f1s = [torch.tensor(f1_score(user_summ, pred_summ)) for user_summ in test_summ]

    if eval_metric == 'avg':
        final_f1 = torch.mean(torch.stack(f1s))
    elif eval_metric == 'max':
        final_f1 = torch.max(torch.stack(f1s))
    else:
